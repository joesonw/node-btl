...
Pagelet.prototype.execute = function* execute(readable,context) {
	...
	if ((typeof this.body) === 'object' && this.body.type === 'jade') {
		var f 	= yield fs.readFile(this.body.path);
		var fn 	= jade.compile(f.toString(),this.body.options || {});
		readable.push(fn(context));
	} else {
	...
	if ((typeof this.footer) === 'object' && this.footer.type === 'jade') {
		var f 	= yield fs.readFile(this.footer.path);
		var fn 	= jade.compile(f.toString(),this.footer.options || {});
		readable.push(fn(context));
	} else {
